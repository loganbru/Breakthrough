...
var grid_start_position : Vector2 = Vector2(0, 0)
...
func _ready():
	grid_start_position = Vector2(0, grid_height * grid_size)
...
func create_grid():
	...
	for y in range(grid_height):
		...
		for x in range(grid_width):
			row.append(false)
		_grid.append(row)
	var _grid_node = Node2D.new()
	_grid_node.position = grid_start_position
	grid_container.add_child(_grid_node)
	...
	for _grid in grid_container.get_children():
		...
		for _brick : Brick in _grid.get_children():
			if _brick.global_position.y < _highest_y:
				if _brick.marked_for_deletion == false:
...
func move_grids_up():
	...
	for grib_obj in grid_objs:
		var _new_pos : Vector2 = grib_obj.node.position + Vector2(0, -move_amount)
		grib_obj.node.position = _new_pos
	
	move_up_counter += move_amount / grid_size
	
	if move_up_counter >= grid_height + 1:
		move_up_counter = 0
		create_grid()
	depth += move_amount / grid_size

func populate_grid(grid_obj_index):
	for y in range(grid_height):
		for x in range(grid_width):
...
func new_game():
	for _grid_obj in grid_objs:
		_grid_obj.container.queue_free()
